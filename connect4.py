...
class Info(Frame):
    def __init__(self, master=None):
        ...
        self.t = Label(self, text="Ghadeer's Turn (Yellow)", font=police)
...
class Point(object):
    def __init__(self, x, y, can, coul="white", bg="red"):
        ...
        self.r = 30
        self.Point = self.can.create_oval(self.x+10,self.y+10,self.x+61,self.y+61,fill=coul,outline="blue")



    def changeColor(self, coul):
        self.can.itemconfigure(self.Point, fill=coul)
        self.coul = coul
        ...
        if self.perm:
            ...
            while lig < len(self.p):
                ...
                if self.p[lig][col].coul == "red" or self.p[lig][col].coul == "yellow":
                    self.p[lig-1][col].changeColor(self.coul)
                    break

                elif lig == len(self.p)-1:
                    self.p[lig][col].changeColor(self.coul)
                    break
            ...
            if self.player == 1:
                self.player = 2
                info.t.config(text="Mai's Turn (Red)")
                self.coul = "red"

            elif self.player == 2:
                self.player = 1
                info.t.config(text="Ghadeer's Turn (Yellow)")
                self.coul = "yellow"
